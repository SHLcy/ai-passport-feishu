...
1. 刷入不含任何飞书凭据的通用完整固件。
2. 手机扫描设备首次配网二维码；设备本地页面同时要求家庭 Wi-Fi、用户自己的 App ID 和 App Secret，其中应用两项强制填写。
3. 页面内再次提示应用创建、发布、可用范围和凭证位置；字段直接通过本地热点写入设备，不上传网站服务器。
...
| 扫码成功但以后无法刷新 | 缺少 `offline_access` 或用户撤销了授权 | 开通权限，解绑后重新授权 |
